...
class BarChart extends HTMLElement {
  constructor() {
    ...
    this.rects = this.data.map((height, index) => {
      ...
      labelDiv.style.left = (index * (rectWidth + gap) + rectWidth / 2) + '%';
    ...
    style.textContent = /*css*/`
      ...
      .container {
        ...
        border-radius: 1rem;
        background: #111827;
        box-shadow: inset 0 0 0.5em rgba(255, 255, 255, 0.05), 0 0 1em rgba(0, 255, 255, 0.05);
        overflow: visible;
      }
  ...
  handleRectHover(rect, labelDiv) {
    ...
    labelDiv.style.transform = 'translate(-50%, -12px)';
    rect.style.fill = 'var(--primary)';
  }

  handleRectOut(rect, labelDiv) {
    labelDiv.style.opacity = '0';
    labelDiv.style.transform = 'translate(-50%, -8px)';
    rect.style.fill = ''; // Reset to original color
  }
}
