...
async function getUserId() {
  ...
  return userData.data.user[0].id;
}
...
function displayDownUpRatio(up, down) {
  ...
  const labelsString = `Down - ${downRatio.toFixed(2)};Up - ${upRatio.toFixed(
    2
  )}`;
  ...
  document.getElementById(
    "pie"
  ).innerHTML = `<pie-chart id="demo" data="${dataString}" gap="0.06" colors="${colors}" donut="0.2" labels="${labelsString}"></pie-chart>`;
}
...
function displayXp(data) {
  ...
  document.getElementById(
    "bar"
  ).innerHTML = `<bar-chart data="${dataString}" colors="${colorsString}" labels="${labelsString}"></bar-chart>`;
}
...
function displayUserData(user) {
  ...
  document
    .getElementById("flag")
    .setAttribute("src", `https://flagsapi.com/${countryCode}/flat/32.png`);
}
